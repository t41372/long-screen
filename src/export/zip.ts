...
export async function* blobChunks(blob: Blob): AsyncGenerator<Uint8Array> { const reader = blob.stream().getReader(); try {
    while (true) {
...
export async function* single(data: Uint8Array): AsyncGenerator<Uint8Array> { yield data; }
function record(size: number): {
    data: Uint8Array;
    view: DataView;
} { const data = new Uint8Array(size); return { data, view: new DataView(data.buffer) }; }
...
export class ZipWriter {
    ...
    private async write(data: Uint8Array): Promise<void> { await this.sink.write(data); this.offset += BigInt(data.byteLength); }
