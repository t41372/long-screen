...
export class BlobReader {
  ...
  async read(offset: number, length: number): Promise<Uint8Array> {
    ...
    if (offset + length > start + this.pageSize) {
      const a = await this.read(offset, start + this.pageSize - offset),
        b = await this.read(start + this.pageSize, length - a.length),
  ...
  async view(offset: number, length: number): Promise<DataView> {
    const a = await this.read(offset, length);
    return new DataView(a.buffer, a.byteOffset, a.byteLength);
  }
  async u32(offset: number): Promise<number> {
    return (await this.view(offset, 4)).getUint32(0);
  }
  async u16(offset: number): Promise<number> {
    return (await this.view(offset, 2)).getUint16(0);
  }
  async u64(offset: number): Promise<number> {
    const v = (await this.view(offset, 8)).getBigUint64(0);
    if (v > BigInt(Number.MAX_SAFE_INTEGER)) {
      throw new Error('Media offset exceeds JavaScript safe-integer precision.');
    }
    return Number(v);
  }
  clear(): void {
