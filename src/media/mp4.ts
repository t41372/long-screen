...
async function* children(r: BlobReader, parent: Box | {
    data: number;
    end: number;
}): AsyncGenerator<Box> {
...
async function child(r: BlobReader, parent: Box, type: string): Promise<Box | undefined> { for await (const b of children(r, parent))
    if (b.type === type)
        return b; }
async function required(r: BlobReader, parent: Box, type: string): Promise<Box> { const b = await child(r, parent, type); if (!b)
    throw new Error(`MP4 ${parent.type} has no ${type}.`); return b; }
const hex = (n: number) => n.toString(16).padStart(2, '0');
function reverse32(n: number): number { let r = 0; for (let i = 0; i < 32; i++) {
    r = (r << 1) | (n & 1);
    n >>>= 1;
} return r >>> 0; }
export class MP4Demuxer implements Demuxer {
    ...
    async init(): Promise<this> {
        ...
        if (elst) {
            ...
            for (let i = 0; i < count; i++) {
                const v = await r.view(elst.data + 8 + i * stride, stride), duration = ver === 1 ? Number(v.getBigUint64(0)) : v.getUint32(0), time = ver === 1 ? Number(v.getBigInt64(8)) : v.getInt32(4), rate = v.getInt16(ver === 1 ? 16 : 8);
                if (rate !== 1)
        ...
        for await (const c of children(r, { data: entry.start + 86, end: entry.end })) {
            if (c.type === 'avcC') {
                ...
                const space = ['', 'A', 'B', 'C'][d[1] >> 6], profile = d[1] & 31, compat = reverse32(v.getUint32(2)).toString(16), tier = d[1] & 32 ? 'H' : 'L';
                const constraint = Array.from(d.subarray(6, 12));
                while (constraint.length && constraint[constraint.length - 1] === 0)
                    constraint.pop();
                codec = `${entry.type}.${space}${profile}.${compat}.${tier}${d[12]}${constraint.length ? '.' + constraint.map(x => hex(x).toUpperCase()).join('.') : ''}`;
            }
            else if (c.type === 'vpcC') {
                ...
                codec = `av01.${d[1] >> 5}.${String(d[1] & 31).padStart(2, '0')}${d[2] & 128 ? 'H' : 'M'}.${d[2] & 64 ? (d[2] & 32 ? '12' : '10') : '08'}`;
            }
            else if (c.type === 'pasp') {
    ...
    private async *tablePackets(): AsyncGenerator<Packet> {
        ...
        const chunkCount = await r.u32(this.offsets.data + 4), scCount = await r.u32(this.stsc.data + 4), ctCount = this.ctts ? await r.u32(this.ctts.data + 4) : 0;
        let tt = 0, ttLeft = 0, delta = 0, ct = 0, ctLeft = 0, composition = 0, ss = 0, nextSync = this.stss ? await r.u32(this.stss.data + 8) : Infinity;
        ...
        for (let chunk = 1; chunk <= chunkCount && sample < count; chunk++) {
            ...
            let offset = this.offsets.type === 'co64' ? await r.u64(this.offsets.data + 8 + (chunk - 1) * 8) : await r.u32(this.offsets.data + 8 + (chunk - 1) * 4);
            for (let k = 0; k < perChunk && sample < count; k++, sample++) {
                ...
                yield { offset, size, timestamp: Math.round(((dts + composition) / this.timescale + this.timeOffset) * 1e6), duration: Math.round(delta / this.timescale * 1e6), key };
                if (key && this.stss) {
                    ss++;
            ...
            for await (const traf of children(r, moof)) {
                ...
                for await (const run of children(r, traf)) {
                    ...
                    for (let i = 0; i < n; i++) {
                        ...
                        yield { offset: dataCursor, size, timestamp: Math.round(((dts + cto) / this.timescale + this.timeOffset) * 1e6), duration: Math.round(duration / this.timescale * 1e6), key: !(sampleFlags & 0x10000) };
                        dataCursor += size;
                        dts += duration;
                    }
                }
