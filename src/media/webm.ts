...
async function uint(r: BlobReader, e: Element): Promise<number> {
  const a = await r.read(e.data, e.end - e.data);
  if (a.length > 8) {
    throw new Error('Oversized EBML integer.');
  }
  let n = 0;
  for (const b of a) {
    n = n * 256 + b;
...
async function* elements(r: BlobReader, p: number, end: number): AsyncGenerator<Element> {
  while (p < end) {
  ...
  private async *block(e: Element, time: number, groupKey?: boolean, blockDuration = 0): AsyncGenerator<Packet> {
    ...
    for (let i = 0; i < count; i++) {
      ...
      yield {
        offset: p,
        size,
        timestamp: Math.round((time + relative) * this.scale / 1000 + i * duration),
        duration,
        key: groupKey ?? !!(flags & 128),
      };
      p += size;
    }
  }
