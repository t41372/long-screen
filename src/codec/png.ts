...
export async function* encodePNG(width: number, height: number, rows: AsyncIterable<Uint8Array> | Iterable<Uint8Array>): AsyncGenerator<Uint8Array> {
...
export async function decodePNG(bytes: Uint8Array): Promise<RGBA> {
    ...
    while (offset + 8 <= bytes.length && !ended) {
        const size = view.getUint32(offset), type = String.fromCharCode(bytes[offset + 4], bytes[offset + 5], bytes[offset + 6], bytes[offset + 7]);
