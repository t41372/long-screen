...
import { type StoredTile, type TileIndex, pngTileCodec } from '../storage/tiles.ts';
import { renderFrame, type Layer, type Scenario } from './world.ts';
export interface Observation {
    frame: number;
    time: number;
    decisions: { canvasId: string; placement: Placement; addedPixels: number; conflictPixels: number; uncertainPixels: number; skipped?: boolean }[];
}
...
export function matchRegion(result: VerifiableRun, layer: Layer, kind: Region['kind'] = 'moving'): Region {
    const candidates = result.regions.filter(r => r.kind === kind).map(r => ({ r, overlap: overlapArea(r.rect, layer.viewport) })).sort((a, b) => b.overlap - a.overlap);
    if (!candidates.length || candidates[0].overlap <= 0)
        throw new Error(`No ${kind} region overlaps layer ${layer.id}. Regions: ${JSON.stringify(result.regions.map(r => ({ id: r.id, kind: r.kind, rect: r.rect })))}`);
...
export async function verifyLayer(result: VerifiableRun, layer: Layer, options: { tolerance?: number } = {}): Promise<LayerReport> {
    const { scenario, atlas, store } = result, region = matchRegion(result, layer), code = atlas.code(region), tolerance = options.tolerance ?? 0;
    ...
    const b = mainCanvas.bounds, bw = Math.max(1, Math.ceil(b.width) + 2), bh = Math.max(1, Math.ceil(b.height) + 2), bx = Math.floor(b.x) - 1, by = Math.floor(b.y) - 1;
    ...
    for await (const { value: t } of iterate<TileIndex>(store, `tile-index/${mainId}/0/`)) {
        ...
        for (let i = 0; i < size * size; i++) {
            ...
            const versions = wx >= 0 && wy >= 0 && wx < layer.world.width && wy < layer.world.height ? layer.world.versions(wx, wy) : [scenario.background];
            const r = pixels[i * 4], g = pixels[i * 4 + 1], bl = pixels[i * 4 + 2];
            const ok = pixels[i * 4 + 3] === 255 && versions.some(c => Math.abs(c[0] - r) <= tolerance && Math.abs(c[1] - g) <= tolerance && Math.abs(c[2] - bl) <= tolerance);
            if (ok)
                continue;
    ...
    return { regionId: region.id, mainCanvas, fragments, framesOnMain, framesOnFragments, skipped, errors, maxError: finite.length ? Math.max(...finite) : 0, meanError: finite.length ? finite.reduce((s, x) => s + x, 0) / finite.length : 0, missing, invented, mismatched, contaminated, covered, expected: expectedCount, regionPixels };
}
/** Static overlays must land in fixed canvases with identical pixels and zero conflicts. */
export async function verifyFixed(result: VerifiableRun, rect: { x: number; y: number; width: number; height: number }): Promise<{ canvas: CanvasMeta; mismatched: number; covered: number; conflicts: number }> {
    ...
    for await (const { value: t } of iterate<TileIndex>(store, `tile-index/${best.id}/0/`)) {
        ...
        for (let i = 0; i < size * size; i++) {
            ...
            if (x < rect.x || y < rect.y || x >= rect.x + rect.width || y >= rect.y + rect.height || atlas.labels[y * scenario.width + x] !== code)
                continue;
            covered++;
