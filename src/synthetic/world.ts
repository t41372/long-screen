...
export class World {
    ...
    picture(r: Rect, seed: number): void {
        ...
        for (let y = 0; y < r.height; y++)
            for (let x = 0; x < r.width; x++) {
                ...
                this.set(r.x + x, r.y + y, [base[0] + 60 * t + n, base[1] + 50 * u + n, base[2] - 40 * t * u + n].map(v => Math.max(0, Math.min(255, v))) as RGB);
            }
        ...
        for (let k = 0; k < 6; k++) {
            ...
            this.fill({ x: r.x + Math.floor(random() * Math.max(1, r.width - w)), y: r.y + Math.floor(random() * Math.max(1, r.height - h)), width: w, height: h }, [random() * 255, random() * 255, random() * 255]);
        }
    }
    ...
    textLine(x: number, y: number, width: number, seed: number, colour: RGB = [48, 50, 46], size = 1): number {
        ...
        while (cx < x + width) {
            ...
            for (let l = 0; l < letters && cx < x + width; l++) {
                const w = Math.round((3 + Math.floor(random() * 4)) * size), ascender = random() < .3 ? Math.round(3 * size) : 0, descender = random() < .15 ? Math.round(3 * size) : 0;
                ...
                if (random() < .5)
                    this.fill({ x: cx + 1, y: y + lineHeight - Math.round(5 * size), width: Math.max(1, w - 2), height: Math.max(1, Math.round(2 * size)) }, [250, 250, 246]);
    ...
    patch(atFrame: number, rect: Rect, draw: (scratch: World) => void): void {
        ...
        for (let y = 0; y < rect.height; y++)
            scratch.data.set(this.data.subarray(((rect.y + y) * this.width + rect.x) * 4, ((rect.y + y) * this.width + rect.x + rect.width) * 4), y * rect.width * 4);
        draw(scratch);
    ...
    for (let col = 0; col < columns; col++) {
        ...
        while (y < height - 40) {
            ...
            if (style === 'cards') {
                ...
                world.fill({ x: x0 + 14, y: y + 16, width: 6, height: 18 }, [[207, 103, 80], [111, 135, 116], [102, 133, 154], [165, 130, 86]][Math.floor(random() * 4)] as RGB);
            ...
            if (roll < .15) {
                ...
                world.picture({ x: x0, y, width: Math.min(w, 260 + Math.floor(random() * (w - 260))), height: Math.min(h, height - y - 8) }, Math.floor(random() * 1e9));
                y += h + 14;
            }
            else if (roll < .42) {
    ...
    for (const layer of s.layers) {
        ...
        for (const d of layer.dynamics || []) {
            ...
            for (let y = 0; y < d.rect.height; y++)
                scratch.data.set(data.subarray(((d.rect.y + y) * layer.world.width + d.rect.x) * 4, ((d.rect.y + y) * layer.world.width + d.rect.x + d.rect.width) * 4), y * d.rect.width * 4);
            d.draw(scratch, index, spec.time);
            for (let y = 0; y < d.rect.height; y++)
                data.set(scratch.data.subarray(y * d.rect.width * 4, (y + 1) * d.rect.width * 4), ((d.rect.y + y) * layer.world.width + d.rect.x) * 4);
        ...
        if (zoom === 1)
            ...
            for (let y = 0; y < v.height; y++)
                for (let x = 0; x < v.width; x++) {
                    ...
                    const wx = p.x + x / zoom, wy = p.y + y / zoom, x0 = Math.floor(wx), y0 = Math.floor(wy), fx = wx - x0, fy = wy - y0, o = ((v.y + y) * width + v.x + x) * 4;
...
export const constantFrames = (count: number, fps = 30, start = 0): Scenario['frames'] => Array.from({ length: count }, (_, i) => ({ time: start + i / fps, duration: 1 / fps }));
