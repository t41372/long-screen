...
export class ScenarioSource implements FrameSource {
    ...
    constructor(readonly scenario: Scenario) {
        ...
        this.info = { name: `demo-${scenario.name}.generated`, size: 0, width: scenario.width, height: scenario.height, codedWidth: scenario.width, codedHeight: scenario.height, rotation: 0, duration: last.time + last.duration, frameCount: scenario.frames.length, codec: 'procedural / lossless pixels', mode: 'Deterministic test input', warnings: ['This is synthetic test material, not evidence of accuracy on arbitrary real recordings.'], notices: [] };
    }
    dispose(): void {
        this.stopped = true;
    }
