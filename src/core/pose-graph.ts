...
export class PoseGraph {
    ...
    nodeId(canvasId: string, frame: number): string { return `${canvasId}/${pad(frame)}`; }
    ...
    async add(canvasId: string, frame: number, p: Point, previous?: PoseNode, weight = 1): Promise<PoseNode> {
        ...
        const id = this.nodeId(canvasId, frame), node: PoseNode = { id, canvasId, frame, x: p.x + (previous ? previous.x - previous.originalX : 0), y: p.y + (previous ? previous.y - previous.originalY : 0), originalX: p.x, originalY: p.y, pinned: !previous, edges: [] };
    ...
    get(id: string): Promise<PoseNode | undefined> { return this.db.get<PoseNode>(`node/${id}`); }
    async connect(aId: string, bId: string, dx: number, dy: number, weight: number, kind: 'odometry' | 'loop'): Promise<void> {
        ...
        await this.db.putMany([{ key: aKey, value: { other: bId, dx, dy, weight, kind } }, { key: bKey, value: { other: aId, dx: -dx, dy: -dy, weight, kind } }]);
        if (kind === 'loop') {
            this.loops++;
        ...
        for (; iterations < 12; iterations++) {
            ...
            for await (const row of iterate<PoseNode>(this.db, 'node/', iterations % 2 === 1)) {
                ...
                for await (const { value: edge } of iterate<PoseEdge>(this.db, `edge/${node.id}/`)) {
                    ...
                    if (!other)
                        continue;
                    const residual = Math.hypot(other.x - node.x - edge.dx, other.y - node.y - edge.dy), weight = edge.weight * Math.min(1, 6 / Math.max(1e-6, residual));
                    x += (other.x - edge.dx) * weight;
                    y += (other.y - edge.dy) * weight;
                    total += weight;
                }
