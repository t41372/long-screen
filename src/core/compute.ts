...
interface BufferLike { mapAsync(mode: number): Promise<void>; getMappedRange(): ArrayBuffer; unmap(): void; destroy(): void; }
interface DeviceLike {
    ...
    createCommandEncoder(): { beginComputePass(): { setPipeline(pipeline: unknown): void; setBindGroup(index: number, group: unknown): void; dispatchWorkgroups(x: number, y: number): void; end(): void }; copyBufferToBuffer(a: BufferLike, aOffset: number, b: BufferLike, bOffset: number, size: number): void; finish(): unknown };
    destroy(): void;
}
export interface GPUProvider { requestAdapter(options?: unknown): Promise<{ requestDevice(): Promise<DeviceLike> } | null>; }
export interface ComputeStats {
    backend: 'CPU' | 'WebGPU box-luma + CPU registration'; reason: string; cpuFrames: number; gpuFrames: number;
...
function deadline<T>(work: Promise<T>, ms: number): Promise<T> {
    ...
    return Promise.race([work, new Promise<T>((_, reject) => { timer = setTimeout(() => reject(new Error('WebGPU operation timed out')), ms); })]).finally(() => clearTimeout(timer));
}
...
export class AnalysisComputer {
    ...
    private buffers?: { input: BufferLike; output: BufferLike; params: BufferLike; read: BufferLike; inputBytes: number; outputBytes: number };
    constructor(private mode: 'auto' | 'cpu' | 'webgpu' = 'auto', private provider: GPUProvider | undefined = (globalThis.navigator as unknown as { gpu?: GPUProvider } | undefined)?.gpu) { this.stats.reason = mode === 'cpu' ? 'CPU requested' : 'CPU until an integer downscale is calibrated'; }
    private async initialize(): Promise<void> {
        ...
        if (this.disabled) { device.destroy(); return; }
        this.device = device;
        void device.lost.then(info => { if (!this.disabled) this.fallback(`Device lost: ${info.message || 'unknown'}`); });
        this.pipeline = await deadline(device.createComputePipelineAsync({ layout: 'auto', compute: { module: device.createShaderModule({ code: BOX_LUMA_WGSL }), entryPoint: 'main' } }), 4000);
    }
    ...
    private fallback(reason: string): void {
        this.disabled = true; this.stats.backend = 'CPU'; this.stats.reason = reason;
        this.releaseBuffers(); this.device?.destroy(); this.device = undefined;
    }
    async gray(image: RGBA, factor: number): Promise<Gray> {
        ...
        if (this.mode === 'cpu' || this.disabled || factor === 1 || factor > 128) {
            this.stats.cpuFrames++; return downscaleGray(image, factor);
        }
        try {
            if (!this.tried) {
                this.tried = true;
                ...
                if (this.mode === 'auto' && gpuMS >= cpuMS * .9) {
                    this.fallback('CPU wins measured transfer + compute + readback'); this.stats.cpuFrames++; return cpu;
                }
                this.stats.backend = 'WebGPU box-luma + CPU registration';
                this.stats.reason = this.mode === 'auto' ? 'Bit-exact and faster in first-frame calibration' : 'Explicit WebGPU, first-frame parity validated';
                this.stats.gpuFrames++; return gpu;
            }
            const result = await this.dispatch(image, factor); this.stats.gpuFrames++; return result;
        } catch (error) {
            this.fallback(String(error)); this.stats.cpuFrames++; return downscaleGray(image, factor);
        }
    }
    private async dispatch(image: RGBA, factor: number): Promise<Gray> {
        ...
        if (Math.max(inputBytes, outputBytes) > Math.min(device.limits.maxStorageBufferBindingSize, device.limits.maxBufferSize)) throw new Error('GPU buffer limit exceeded');
        if (!this.buffers || this.buffers.inputBytes !== inputBytes || this.buffers.outputBytes !== outputBytes) {
            this.releaseBuffers();
            const input = device.createBuffer({ size: inputBytes, usage: 128 | 8 }), output = device.createBuffer({ size: outputBytes, usage: 128 | 4 });
        ...
        const bindings = device.createBindGroup({ layout: this.pipeline!.getBindGroupLayout(0), entries: [input, output, params].map((buffer, binding) => ({ binding, resource: { buffer } })) });
        const encoder = device.createCommandEncoder(), pass = encoder.beginComputePass();
        pass.setPipeline(this.pipeline); pass.setBindGroup(0, bindings); pass.dispatchWorkgroups(Math.ceil(width / 16), Math.ceil(height / 16)); pass.end();
        encoder.copyBufferToBuffer(output, 0, read, 0, outputBytes); device.queue.submit([encoder.finish()]);
        await deadline(read.mapAsync(1), 4000);
        try { return { width, height, data: Uint8Array.from(new Uint32Array(read.getMappedRange())) }; }
        finally { read.unmap(); }
    }
    dispose(): void { this.disabled = true; this.releaseBuffers(); this.device?.destroy(); this.device = undefined; }
}
