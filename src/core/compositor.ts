import type { RGBA, Region, Placement, CanvasMeta, Rect, Diagnostic } from '../types.ts';
import type { KV } from '../storage/db.ts';
import { iterate } from '../storage/db.ts';
import { type TileStore, QUALITY_BLOCK, covered, markCovered } from '../storage/tiles.ts';
...
export class Compositor {
    ...
    async add(image: RGBA, region: Region, p: Placement, frame: number, meta: CanvasMeta): Promise<CompositeStats> {
        ...
        if (conflictBlocks.size) {
            ...
            await this.emit({ code: 'TEMPORAL_OR_ALIGNMENT_CONFLICT', severity: 'warning', time: p.time, frame, canvasId: p.canvasId, region: world, confidence: p.confidence,
                message: `对齐后的重叠区域有 ${stats.conflicts.toLocaleString()} 个明显不同的像素。可能是动画、内容更新、重排或配准残差。`,
                action: this.policy === 'stable' ? '已尽量冻结单一时刻的完整冲突区域；查看橙色诊断和原视频时间点。' : '仅在完整可见时用同一帧更新整块冲突区域；并非全页面同一时刻。' });
        }
    ...
    private async resolveTemporal(image: RGBA, region: Region, p: Placement, frame: number, rect: Rect, visible: Rect): Promise<void> {
        ...
        const expanded = !old || merged.x !== old.rect.x || merged.y !== old.rect.y || merged.width !== old.rect.width || merged.height !== old.rect.height;
        const choose = complete && (!old || !old.complete || expanded || this.policy === 'latest');
        const record: TemporalRegion = old || { id: pad(this.temporalSequence++), canvasId: p.canvasId, rect: merged, chosenFrame: frame, chosenTime: p.time, complete: false, revisions: 0 };
        record.rect = merged;
        if (expanded)
        ...
        if (!record.complete && (!old || expanded))
            await this.emit({ code: 'INCOMPLETE_TEMPORAL_PATCH', severity: 'warning', canvasId: p.canvasId, frame, time: p.time, region: merged,
                message: '这个变化区域从未完整地出现在一个可用视口中；无法保证其所有像素来自同一时刻。', action: '保留已观察内容和明确冲突标记，没有填造未观察部分。' });
    }
