...
export class KeyframeIndex {
    ...
    async find(q: RelocalizationQuery): Promise<Relocalization | undefined> {
        ...
        if (features.length < 8)
            return;
        const allWords = featureWords(features.filter((_, i) => i % 2 === 0)), words = allWords.filter((_, i) => i % Math.max(1, Math.floor(allWords.length / 48)) === 0).slice(0, 48), votes = new Map<string, number>();
        let truncated = false;
        ...
        if (truncated)
            await this.warn('A highly repetitive visual-word posting exceeded the 96-candidate retrieval budget. Relocalization is approximate here; all image observations remain in the reconstruction journal.');
        const candidates = [...votes.entries()].sort((a, b) => b[1] - a[1]).slice(0, 12), results: (Relocalization & { strong: boolean })[] = [];
        for (const [id, count] of candidates) {
            ...
            for (const m of models.slice(0, 8)) {
                ...
                results.push({ keyframe: k, offset: { x: refined.x, y: refined.y }, confidence, ambiguous: m.ambiguous, support: m.support, unique: m.unique, error: refined.error, analysisError: audit.error, strong });
            }
        }
        ...
        const rival = results.find(r => r !== best && (position(r).canvas !== bp.canvas || Math.hypot(position(r).x - bp.x, position(r).y - bp.y) > 6) && score(r) > score(best) * .8);
