...
import { TileStore, covered, markCovered } from '../storage/tiles.ts';
...
export interface FrameLayout {
    width: number; height: number; pane: Rect; content: Rect; dx: number; dy: number; seamX: number;
}
export function frameLayout(source: RGBA, region: Region, canvas: CanvasMeta): FrameLayout {
    const pane = region.crop || region.rect;
    const content = { x: pane.x, y: pane.y, width: Math.max(pane.width, Math.ceil(canvas.bounds.width)), height: Math.max(pane.height, Math.ceil(canvas.bounds.height)) };
    return { width: source.width + content.width - pane.width, height: source.height + content.height - pane.height,
        pane, content, dx: content.width - pane.width, dy: content.height - pane.height, seamX: Math.floor(pane.x + pane.width / 2) };
}
...
function mode(values: number[]): number {
    const counts = new Map<number, number>(); let best = values[0] || 0, count = 0;
    for (const v of values) { const n = (counts.get(v) || 0) + 1; counts.set(v, n); if (n > count) { count = n; best = v; } }
    return best;
}
...
export async function buildFramedCanvas(store: KV, tiles: TileStore, sourceCanvas: CanvasMeta, region: Region, regions: Region[], checkpoint: () => Promise<void>): Promise<CanvasMeta | undefined> {
    ...
    const meta: CanvasMeta = { ...sourceCanvas, id: `${sourceCanvas.id}-framed`, name: `${sourceCanvas.name} · 保留外框`, kind: 'presentation',
        bounds: { x: 0, y: 0, width: layout.width, height: layout.height }, tileCount: 0, maxLevel: 0, observedPixels: 0, uncertainPixels: 0, conflictPixels: 0,
        presentation: { sourceCanvas: sourceCanvas.id, sourceRegion: pane, referenceFrame: reference.frame,
            offset: { x: content.x - sourceCanvas.bounds.x, y: content.y - sourceCanvas.bounds.y },
            extension: 'Native-scale reference chrome shown once; flat background-only extensions are presentation, NOT observed world content. Other panes in the frame are reference snapshots, NOT merged trajectories. Coverage bits mark copied evidence; opaque extension pixels are deliberately not covered.' } };
    const bg = backgrounds(source, pane), sourcePixels = new Uint32Array(source.data.buffer, source.data.byteOffset, source.data.length / 4), size = tiles.size;
    ...
    for (let ty = 0; ty < Math.ceil(layout.height / size); ty++) for (let tx = 0; tx < Math.ceil(layout.width / size); tx++) {
        await checkpoint();
        const tile = await tiles.get(meta.id, tx, ty), dst = new Uint32Array(tile.pixels.buffer, tile.pixels.byteOffset, tile.pixels.length / 4);
        const bounds = { x: tx * size, y: ty * size, width: Math.min(size, layout.width - tx * size), height: Math.min(size, layout.height - ty * size) };
        ...
        if (overlap.width > 0 && overlap.height > 0) {
            ...
            for (let sy = Math.floor(wy / size); sy <= Math.floor((wy + overlap.height - 1) / size); sy++) for (let sx = Math.floor(wx / size); sx <= Math.floor((wx + overlap.width - 1) / size); sx++) {
                const raw = await tiles.get(sourceCanvas.id, sx, sy);
                const part = intersect({ x: wx, y: wy, width: overlap.width, height: overlap.height }, { x: sx * size, y: sy * size, width: size, height: size });
        ...
        if (tile.pixels.some((a, i) => i % 4 === 3 && a !== 0)) {
            tile.dirty = true; meta.tileCount++;
