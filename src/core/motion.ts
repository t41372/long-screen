...
export function resampleGray(g: Gray, scale: number): Gray {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      const sx = Math.min(g.width - 1.001, x / scale),
        sy = Math.min(g.height - 1.001, y / scale),
        x0 = Math.floor(sx),
        y0 = Math.floor(sy),
        fx = sx - x0,
        fy = sy - y0,
        i = y0 * g.width + x0;
      data[y * width + x] = (g.data[i] * (1 - fx) + g.data[i + 1] * fx) * (1 - fy) +
        (g.data[i + g.width] * (1 - fx) + g.data[i + g.width + 1] * fx) * fy;
    }
  }
