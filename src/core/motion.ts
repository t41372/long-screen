import type { Gray, Feature, Match, Motion, MotionField, Rect, Point, RGBA } from '../types.ts';
import { clamp, median, norm, rng } from './math.ts';
import { extractFeatures, matchFeatures, meanDifference } from './features.ts';
export function translationHypotheses(matches: Match[], max = 6): Motion[] {
    ...
    for (const seed of seeds) {
        ...
        const confidence = clamp((1 - Math.exp(-support.length / 9)) * (0.48 + 0.52 * unique / support.length) * Math.min(1, spread / 4), 0, 0.99);
...
export function verifyTranslation(a: Gray, b: Gray, dx: number, dy: number, roi?: Rect): number {
    ...
    return errors.slice(0, Math.max(2, Math.ceil(errors.length * .75))).reduce((s, x) => s + x, 0) / Math.max(2, Math.ceil(errors.length * .75));
}
...
export function auditTranslation(a: Gray, b: Gray, dx: number, dy: number, roi?: Rect, tolerant = false): Audit {
    ...
    dx = Math.round(dx);
    dy = Math.round(dy);
    const x0 = Math.max(2, Math.ceil(r.x), Math.ceil(r.x - dx)), y0 = Math.max(2, Math.ceil(r.y), Math.ceil(r.y - dy));
    const x1 = Math.min(b.width - 2, a.width - dx - 2, r.x + r.width, r.x + r.width - dx), y1 = Math.min(b.height - 2, a.height - dy - 2, r.y + r.height, r.y + r.height - dy);
    ...
    for (let y = y0; y < y1; y += 2)
        for (let x = x0; x < x1; x += 2) {
            ...
            const gb = Math.abs(b.data[i + 1] - b.data[i - 1]) + Math.abs(b.data[i + b.width] - b.data[i - b.width]), ga = Math.abs(a.data[j + 1] - a.data[j - 1]) + Math.abs(a.data[j + a.width] - a.data[j - a.width]);
            if (Math.max(ga, gb) < 20)
                continue;
            let difference = Math.abs(a.data[j] - b.data[i]);
            if (tolerant)
                difference = Math.min(difference, Math.abs(a.data[j + 1] - b.data[i]), Math.abs(a.data[j - 1] - b.data[i]), Math.abs(a.data[j + a.width] - b.data[i]), Math.abs(a.data[j - a.width] - b.data[i]));
    ...
    return { error: n >= 24 ? error / n : Infinity, mismatch: n ? bad / n : 1, overlap, samples: n, blocks, agreeing, agreement: blocks ? agreeing / blocks : 0, agreeingError: agreeing ? agreeingError / agreeing : Infinity };
}
...
export function estimateMotion(a: Gray, b: Gray, _previous?: MotionField, af?: Feature[], bf?: Feature[]): MotionField {
    ...
    if (difference < .12) {
        return { motions: [{ x: 0, y: 0, support: 0, unique: 0, confidence: .98, error: difference, ambiguous: false }], labels: new Uint8Array(n), confidence: new Uint8Array(n).fill(245), dynamic: new Uint8Array(n), cols, rows, cell, difference, featureCount: 0, unknown: false, zoom: 1 };
    }
    ...
    for (let cy = 0; cy < rows; cy++)
        for (let cx = 0; cx < cols; cx++) {
            const idx = cy * cols + cx, r = { x: cx * cell, y: cy * cell, width: Math.min(cell, b.width - cx * cell), height: Math.min(cell, b.height - cy * cell) };
    ...
    return { motions, labels, confidence, dynamic, cols, rows, cell, difference, featureCount: featuresB.length, unknown, zoom: detectScale(matches) };
}
...
export function refineNative(a: RGBA, b: RGBA, guess: Point, region: Rect, mask?: (x: number, y: number) => boolean, radius = 3): NativeRefinement {
    if (a.width !== b.width || a.height !== b.height)
    ...
    const cost = (dx: number, dy: number): number => {
        ...
        for (const p of points) {
            ...
            error += Math.min(90, (Math.abs(a.data[j] - b.data[i]) + Math.abs(a.data[j + 1] - b.data[i + 1]) + Math.abs(a.data[j + 2] - b.data[i + 2])) / 3);
            n++;
        }
    ...
    for (let y = Math.round(guess.y) - radius; y <= Math.round(guess.y) + radius; y++)
        for (let x = Math.round(guess.x) - radius; x <= Math.round(guess.x) + radius; x++) {
            const e = cost(x, y);
            costs.push({ x, y, e });
            if (e < best - 1e-6 || (Math.abs(e - best) < 1e-6 && Math.hypot(x - guess.x, y - guess.y) < Math.hypot(p.x - guess.x, p.y - guess.y))) {
                best = e;
                p = { x, y };
            }
        }
...
export function extractPatches(native: Gray, region: Rect, features: Point[], factor: number, count = 24, size = 32): Patch[] {
    const out: Patch[] = [], rx = Math.round(region.x), ry = Math.round(region.y), rw = Math.round(region.width), rh = Math.round(region.height);
...
export function refinePatches(patches: Patch[], native: Gray, region: Rect, guess: Point, radius = 3): NativeRefinement {
    const rx = Math.round(region.x), ry = Math.round(region.y), rw = Math.round(region.width), rh = Math.round(region.height), w = native.width;
...
export function resampleGray(g: Gray, scale: number): Gray {
    const width = Math.max(2, Math.round(g.width * scale)), height = Math.max(2, Math.round(g.height * scale)), data = new Uint8Array(width * height);
    for (let y = 0; y < height; y++)
        for (let x = 0; x < width; x++) {
            const sx = Math.min(g.width - 1.001, x / scale), sy = Math.min(g.height - 1.001, y / scale), x0 = Math.floor(sx), y0 = Math.floor(sy), fx = sx - x0, fy = sy - y0, i = y0 * g.width + x0;
            data[y * width + x] = (g.data[i] * (1 - fx) + g.data[i + 1] * fx) * (1 - fy) + (g.data[i + g.width] * (1 - fx) + g.data[i + g.width + 1] * fx) * fy;
        }
...
export function probeScale(previous: Gray, current: Gray, currentFeatures: Feature[], scales = [1.1, 1.25, 1.5, 2, 1 / 1.1, 1 / 1.25, 1 / 1.5, 1 / 2]): { scale: number; error: number } | undefined {
    let best: { scale: number; error: number } | undefined;
    for (const scale of scales) {
        const scaled = resampleGray(previous, scale), features = extractFeatures(scaled, 320), matches = matchFeatures(features, currentFeatures);
