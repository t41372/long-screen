...
import { iterate, Namespace } from '../storage/db.ts';
  ...
  async run(): Promise<Project> {
    ...
    try {
      await this.diagnostics.emit({
        ...
        severity: 'info',
        message: '重建采用分层平移画布与几何回环约束。自动遮罩和动态区域属于启发式推断；置信分数不是经过校准的正确概率。',
    ...
    try {
      for await (const frame of this.source.frames()) {
        ...
        const record: ScanRecord = {
          ...
          field,
          features: duplicate ? undefined : features,
          duplicate,
        };
    ...
    for await (const frame of this.source.frames()) {
      ...
      for (const state of states) {
        ...
        if (r.kind === 'moving' && !state.started) {
          ...
          const matches = matchFeatures(state.previousFeatures || [], ownFeatures),
            models = translationHypotheses(matches, 16).filter((m) => m.support >= 4);
          ...
          if (decision === 'tracked') {
            ...
            if (match && !match.ambiguous && match.confidence > .6 && !zoomChange) {
              ...
              const scale = zoomChange
                ...
                ? probeScale(previousGray, g, ownFeatures)
                : undefined;
              state.fragment++;
              state.pose = { x: 0, y: 0 };
              state.lastNode = undefined;
          ...
          if (needsKey) {
            ...
            if ((relocalized && oldAnchor) || attachedFrom) {
              ...
              const discrepancy = Math.hypot(
                global.keyframe.x + global.offset.x - state.pose.x,
                global.keyframe.y + global.offset.y - state.pose.y,
              );
  ...
  private async render(): Promise<void> {
    ...
    for await (const frame of this.source.frames()) {
      ...
      for (const placement of plan.placements) {
        ...
        const correction = await graph.correction(placement.node, frame.index),
          shift = this.attachmentShift(attachments, placement.canvasId);
    ...
    await this.store.put('memory-stats', {
      ...
      note: 'Codec/browser/GPU allocations are additional, not a hard process-RSS bound.',
    });
    await this.persist();
  }
}
