...
import { loopClosureVerdict, needsKeyframe, targetPose, thinOverlapCorrection } from './track.ts';
...
export async function keyframeStep(pass: SolvePass, state: RegionState, input: KeyframeStepInput): Promise<void> {
  ...
  const needsKey = needsKeyframe(
    r.kind,
    !!state.lastNode,
    anchorDistance,
    r.rect,
    frame.index - (state.lastNode?.frame ?? -Infinity),
    scan.field.difference,
  );
  if (!needsKey) {
    return;
  }
  ...
  if (global && resolveTarget(global.keyframe.canvasId) !== state.canvasId && !global.ambiguous && global.confidence > .72) {
    const target = resolveTarget(global.keyframe.canvasId), shift = attachmentShift(attachments, global.keyframe.canvasId);
    const pose = targetPose(global.keyframe, global.offset, shift);
  ...
  if ((relocalized && oldAnchor) || attachedFrom) {
    ...
    const verdict = loopClosureVerdict(global.ambiguous, global.confidence, discrepancy);
    if (verdict.verdict === 'closure') {
      await graph.connect(
        global.keyframe.node,
        node.id,
    ...
    } else if (verdict.verdict === 'inconsistent') {
      await ctx.diagnostics.emit({
