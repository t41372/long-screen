 ...
 * closures (they close over `attachments`, which grows during the pass), and the analysis-geometry constants. */
export interface SolvePass {
  ...
  canonicalCanvas(id: string): string;
 ...
 * region's Placement, calling keyframe-step.ts's keyframeStep() for the keyframe/revisit half. Mutates `state` and
