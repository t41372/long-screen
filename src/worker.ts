...
import { Database, deletePrefix, Namespace } from './storage/db.ts';
...
async function dispatch(type: string, payload: Record<string, any>): Promise<unknown> {
  ...
  if (type === 'start') {
    ...
    if (!['auto', 'cpu', 'webgpu'].includes(settings.compute || 'cpu') || !['context', 'region'].includes(settings.framing || 'region')) {
    ...
    const engine = new Engine(database, source, settings, {
      ...
      preview: (blob) => post({ event: 'preview', blob }),
