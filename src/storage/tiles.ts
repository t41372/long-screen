        ...
        for (let level = 1; level <= maxLevel; level++) {
            ...
            for await (const row of iterate<{ x: number; y: number }>(this.db, `pyramid-todo/${meta.id}/${level}/`)) {
                ...
                if (any) {
                    ...
                    await this.db.putMany([{ key: `tile/${key}`, value: { blob, x, y, level, coverage: new Uint8Array(), owner: new Uint32Array(), score: new Float32Array() } }, { key: `tile-index/${key}`, value: { canvasId: meta.id, level, x, y, observed: 0 } satisfies TileIndex }]);
                }
