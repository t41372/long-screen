...
export class Database implements KV {
    ...
    static async open(): Promise<Database> {
        ...
        const db = await new Promise<IDBDatabase>((resolve, reject) => {
            ...
            request.onblocked = () => reject(new Error('Local database upgrade is blocked by another Long Screen tab. Close that tab and retry.'));
        });
    ...
    async get<T>(key: string): Promise<T | undefined> {
        ...
        return new Promise((resolve, reject) => { r.onsuccess = () => resolve(r.result?.value as T | undefined); r.onerror = () => reject(r.error); });
    }
    async put(key: string, value: unknown): Promise<void> { await this.putMany([{ key, value }]); }
    async putMany(rows: Row[]): Promise<void> {
        if (!rows.length)
            return;
        const tx = this.db.transaction('records', 'readwrite'), store = tx.objectStore('records');
        const done = new Promise<void>((resolve, reject) => { tx.oncomplete = () => resolve(); tx.onerror = () => reject(tx.error || new Error('Local storage transaction failed.')); tx.onabort = () => reject(tx.error || new Error('Local storage transaction aborted.')); });
    ...
    async scan<T>(prefix: string, options: {
        ...
        const range = reverse ? IDBKeyRange.bound(prefix, after || prefix + '\uffff', false, !!after) : IDBKeyRange.bound(after || prefix, prefix + '\uffff', !!after, false);
...
export class Namespace implements KV {
    ...
    get<T>(key: string): Promise<T | undefined> { return this.base.get<T>(this.prefix + key); }
    put(key: string, value: unknown): Promise<void> { return this.base.put(this.prefix + key, value); }
    putMany(rows: Row[]): Promise<void> { return this.base.putMany(rows.map(r => ({ key: this.prefix + r.key, value: r.value }))); }
    delete(key: string): Promise<void> { return this.base.delete(this.prefix + key); }
    async scan<T>(prefix: string, options: {
        after?: string;
        limit?: number;
        reverse?: boolean;
    } = {}): Promise<Row<T>[]> {
        const rows = await this.base.scan<T>(this.prefix + prefix, { ...options, after: options.after ? this.prefix + options.after : undefined });
