import { DEFAULT_SETTINGS, type Project, type CanvasMeta, type Diagnostic, type Progress, type Region, type MediaInfo } from '../types.ts';
...
function rpc<T = any>(type: string, payload: Record<string, unknown> = {}): Promise<T> { return new Promise((resolve, reject) => { const id = ++requestId; requests.set(id, { resolve, reject }); worker.postMessage({ id, type, payload }); }); }
let selectedFile: File | undefined, videoURL: string | undefined, previewURL: string | undefined, firstBitmap: ImageBitmap | undefined, mediaInfo: MediaInfo | undefined;
let project: Project | undefined, canvases: CanvasMeta[] = [], busy = false, paused = false, diagnosticRows: Diagnostic[] = [], diagnosticCursor: string | undefined, projectCursor: string | undefined, refreshing = false, lastRefresh = 0;
let manualRegions: Region[] = [], draftRegions: Region[] = [], draftStart: {
    x: number;
    y: number;
...
function waitVideo(event: string, timeout = 20000): Promise<void> { return new Promise((resolve, reject) => { const cleanup = () => { clearTimeout(timer); video.removeEventListener(event, done); video.removeEventListener('error', failed); }; const done = () => { cleanup(); resolve(); }, failed = () => { cleanup(); reject(new Error(video.error?.message || 'Native media playback failed.')); }; const timer = setTimeout(() => { cleanup(); reject(new Error(`Native video ${event} timed out.`)); }, timeout); video.addEventListener(event, done, { once: true }); video.addEventListener('error', failed, { once: true }); }); }
async function seek(time: number): Promise<void> { if (video.readyState >= 2 && Math.abs(video.currentTime - time) < .00001)
    return; const done = waitVideo('seeked'); video.currentTime = Math.min(Math.max(0, time), Math.max(0, video.duration - .00001)); await done; if (video.readyState < 2)
...
async function chooseFile(file: File): Promise<void> {
    ...
    void waitVideo('loadeddata').then(() => { nativeReady = Number.isFinite(video.duration) && video.duration > 0; }).catch(() => { nativeReady = false; });
    const chosen = file;
    try {
        ...
        $('file-subtitle').textContent = `${mediaInfo.width} × ${mediaInfo.height} · ${timeText(mediaInfo.duration)} · ${mediaInfo.frameCount ?? '?'} 帧 · ${mediaInfo.codec} · ${humanBytes(file.size)}`;
        ...
        if (selectedFile !== chosen)
            return;
        addDiagnostic({ code: 'PROBE_FAILED', severity: 'warning', message: `逐帧解码探测失败：${String(error)}`, action: '将尝试浏览器原生播放器读取元数据。若仍要处理，请在解码方式中选择“近似 · 原生 seek”。' });
    }
    try {
        ...
        nativeReady = true;
        mediaInfo = { name: file.name, size: file.size, width: video.videoWidth, height: video.videoHeight, codedWidth: video.videoWidth, codedHeight: video.videoHeight, rotation: 0, duration: video.duration, codec: 'native media element', mode: 'Native seek', warnings: [], notices: [] };
        await seek(0);
        firstBitmap = await captureNativeFrame();
        $('file-subtitle').textContent = `${mediaInfo.width} × ${mediaInfo.height} · ${timeText(mediaInfo.duration)} · ${humanBytes(file.size)} · 仅原生播放器可读`;
        $<HTMLButtonElement>('regions-btn').disabled = false;
    }
    catch (error) {
...
function addDiagnostic(d: Diagnostic): void { diagnosticRows.push(d); if (diagnosticRows.length > 180)
    ...
    diagRefresh = true;
    setTimeout(() => { diagRefresh = false; renderDiagnostics(); }, 160);
} }
function renderDiagnostics(): void {
    const container = $('diagnostics');
    const filter = $<HTMLSelectElement>('diagnostic-filter').value, rows = diagnosticRows.filter(d => filter !== 'issues' || d.severity !== 'info');
...
async function loadDiagnostics(reset = true): Promise<void> { if (!project)
    return; if (reset) {
    diagnosticRows = [];
...
async function openProject(id: string): Promise<void> { if (busy) {
    toast('请先完成当前处理。');
    return;
} resetView(); const result = await rpc('open', { projectId: id }); project = result.project; updateProject(project!); await refreshCanvases(true); await loadDiagnostics(); viewer.fit(); setBusy(false); $('status-title').textContent = result.interrupted ? '处理曾中断 · 查看已提交结果' : phaseNames[project!.status]; $('progress-message').textContent = project!.error || '本地结果已恢复；原视频没有复制进项目。'; $('progress-count').textContent = `${project!.renderedFrames} 帧`; if (result.interrupted)
    addDiagnostic({ code: 'INTERRUPTED_SESSION', severity: 'warning', message: '上一次处理没有完成。仅已提交的数据可恢复；重新处理需要再次选择原视频，当前不支持断点续算。' }); }
async function history(reset = true): Promise<void> { try {
    if (reset) {
    ...
    for (const row of rows) {
        ...
        open.textContent = '打开';
        open.onclick = () => { void openProject(p.id).then(() => $<HTMLDialogElement>('history-dialog').close()).catch(e => toast(String(e), true)); };
        const remove = document.createElement('button');
        remove.className = 'quiet';
        remove.textContent = '删除';
        remove.onclick = () => { if (confirm(`删除本地项目「${p.name}」及其所有瓦片？`))
            void rpc('delete', { projectId: p.id }).then(() => { item.remove(); if (project?.id === p.id) {
                project = undefined;
                resetView();
                setBusy(false);
            } void storageInfo(); }).catch(e => toast(String(e), true)); };
...
function drawRegions(): void {
    ...
    const draw = (r: Region['rect'], kind: string, n?: number) => { ctx.fillStyle = kind === 'ignore' ? '#a33b3b33' : kind === 'fixed' ? '#ad8b3d33' : '#55966a33'; ctx.strokeStyle = kind === 'ignore' ? '#ba6455' : kind === 'fixed' ? '#bf9b4b' : '#638f63'; ctx.lineWidth = 2; ctx.fillRect(r.x * scale, r.y * scale, r.width * scale, r.height * scale); ctx.strokeRect(r.x * scale, r.y * scale, r.width * scale, r.height * scale); if (n !== undefined) {
    ...
    draftRegions.forEach((r, i) => { const button = document.createElement('button'); button.textContent = `${i + 1}. ${r.name} ${r.rect.width}×${r.rect.height} ×`; button.onclick = () => { draftRegions.splice(i, 1); drawRegions(); }; list.append(button); });
}
function regionPoint(e: PointerEvent): {
    x: number;
    y: number;
} { const rect = $('region-canvas').getBoundingClientRect(); return { x: Math.round(Math.max(0, Math.min(firstBitmap!.width, (e.clientX - rect.left) * firstBitmap!.width / rect.width))), y: Math.round(Math.max(0, Math.min(firstBitmap!.height, (e.clientY - rect.top) * firstBitmap!.height / rect.height))) }; }
async function doExport(format: 'project' | 'png'): Promise<void> {
    ...
    try {
        const width = Math.min(4096, Math.max(512, Math.floor(project.settings.memoryMB * 1024 * 1024 * .20 / (project.settings.tileSize * 4) / 512) * 512)), single = c && c.bounds.width <= width && c.bounds.height <= 32767 && c.bounds.width * c.bounds.height <= 100000000;
        ...
        if ('showSaveFilePicker' in window)
            try {
                ...
                }).showSaveFilePicker({ suggestedName: name, types: [{ description: name.endsWith('.png') ? 'PNG image' : 'ZIP64 archive', accept: { [name.endsWith('.png') ? 'image/png' : 'application/zip']: [name.endsWith('.png') ? '.png' : '.zip'] } }] });
            }
            catch (error) {
        ...
        if (result.blob) {
            ...
            if (result.temporary) {
                ...
                cleanup.onclick = () => { void rpc('cleanup-export', { key: result.temporary }).then(() => { if (downloadURL)
                    URL.revokeObjectURL(downloadURL); $('export-download').replaceChildren(); void storageInfo(); }).catch(e => toast(String(e), true)); };
        ...
        toast(String(error), true);
        addDiagnostic({ code: 'EXPORT_ERROR', severity: 'error', message: String(error), action: '没有把未完成的导出标记为成功。已提交的项目瓦片仍在本机。' });
    }
    finally {
        setBusy(false);
    }
...
worker.onmessage = event => {
    ...
    if (m.event === 'frame-request') {
        void seek(m.time).then(() => captureNativeFrame()).then(bitmap => worker.postMessage({ type: 'frame-response', id: m.id, bitmap }, [bitmap])).catch(error => worker.postMessage({ type: 'frame-response', id: m.id, error: String(error) }));
        return;
    }
    if (m.event === 'progress')
        progress(m.data);
...
worker.onerror = event => { for (const pending of requests.values())
    pending.reject(new Error(event.message)); requests.clear(); setBusy(false); toast(`Worker 错误：${event.message}`, true); };
worker.onmessageerror = () => toast('Worker 消息无法解码。请保留现有结果并重新加载。', true);
$<HTMLInputElement>('file-input').onchange = e => { const file = (e.target as HTMLInputElement).files?.[0]; if (file)
    void chooseFile(file); };
const drop = $('drop-zone');
drop.onkeydown = e => { if (e.key === 'Enter' || e.key === ' ')
    $<HTMLInputElement>('file-input').click(); };
drop.ondragover = e => { e.preventDefault(); drop.classList.add('dragging'); };
drop.ondragleave = () => drop.classList.remove('dragging');
drop.ondrop = e => { e.preventDefault(); drop.classList.remove('dragging'); const file = e.dataTransfer?.files[0]; if (file)
    void chooseFile(file); };
$('start-btn').onclick = () => void start();
$('demo-btn').onclick = () => void start($<HTMLSelectElement>('demo-select').value);
$('pause-btn').onclick = () => { void rpc('pause', { paused: !paused }).then(result => { paused = result.paused; $('pause-btn').textContent = paused ? '继续' : '暂停'; $('status-title').textContent = paused ? '已暂停 · 内存状态保留' : '继续处理'; }).catch(e => toast(String(e), true)); };
$('stop-btn').onclick = () => { void rpc('stop').then(() => toast('正在结束当前阶段，并尽可能合成、保存已处理的观察。')).catch(e => toast(String(e), true)); };
...
$('history-btn').onclick = () => { $<HTMLDialogElement>('history-dialog').showModal(); void history(); };
...
for (const el of document.querySelectorAll<HTMLElement>('[data-close]'))
    el.onclick = () => { $<HTMLDialogElement>(el.dataset.close!).close(); video.pause(); };
$('persist-btn').onclick = () => { void navigator.storage.persist().then(granted => toast(granted ? '浏览器已授予持久存储；清除网站数据仍会删除项目。' : '浏览器未授予持久存储。请导出重要结果，避免自动回收。')).catch(error => toast(String(error), true)); };
$('regions-btn').onclick = () => { draftRegions = structuredClone(manualRegions); draftRect = undefined; $<HTMLDialogElement>('regions-dialog').showModal(); drawRegions(); };
const regionCanvas = $<HTMLCanvasElement>('region-canvas');
regionCanvas.onpointerdown = e => { if (!firstBitmap)
    return; draftStart = regionPoint(e); regionCanvas.setPointerCapture(e.pointerId); };
regionCanvas.onpointermove = e => { if (!draftStart)
    return; const p = regionPoint(e); draftRect = { x: Math.min(p.x, draftStart.x), y: Math.min(p.y, draftStart.y), width: Math.abs(p.x - draftStart.x), height: Math.abs(p.y - draftStart.y) }; drawRegions(); };
regionCanvas.onpointerup = () => { if (draftRect && draftRect.width >= 4 && draftRect.height >= 4) {
    const kind = $<HTMLSelectElement>('region-kind').value as Region['kind'];
    draftRegions.push({ id: `manual-${draftRegions.length}`, name: kind === 'moving' ? `内容区域 ${draftRegions.length + 1}` : kind === 'fixed' ? '固定界面' : '主动忽略', kind, rect: draftRect, manual: true });
} draftStart = undefined; draftRect = undefined; drawRegions(); };
regionCanvas.onpointercancel = () => { draftStart = undefined; draftRect = undefined; drawRegions(); };
$('clear-regions').onclick = () => { draftRegions = []; drawRegions(); };
$('save-regions').onclick = () => { manualRegions = structuredClone(draftRegions); $('regions-count').textContent = manualRegions.length ? `${manualRegions.length} 个指定区域 ↗` : '自动识别 ↗'; $<HTMLDialogElement>('regions-dialog').close(); };
$('export-project').onclick = () => void doExport('project');
$('export-png').onclick = () => void doExport('png');
globalThis.addEventListener('beforeunload', e => { if (busy) {
    e.preventDefault();
    e.returnValue = '';
} });
void rpc('capabilities').then(c => { if (!c.offscreen)
    toast('当前浏览器缺少 OffscreenCanvas；无法运行渲染 Worker。', true); if (!c.webcodecs)
    toast('当前浏览器没有 WebCodecs。需要明确选择可能漏帧的兼容 seek 模式。'); }).catch(error => toast(`无法打开本地数据库：${String(error)}`, true));
void storageInfo();
