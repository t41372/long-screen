...
export class TiledViewer {
    ...
    canvas.addEventListener('pointermove', (e) => {
      ...
      if (before.length === 1) {
        ...
        const a = { x: (before[0].x + before[1].x) / 2, y: (before[0].y + before[1].y) / 2 },
          b = { x: (after[0].x + after[1].x) / 2, y: (after[0].y + after[1].y) / 2 };
  ...
  private moveTo(scale: number, ox: number, oy: number, animate: boolean): void {
    ...
    const step = (now: number) => {
      ...
      this.ox = from.x + (ox - from.x) * ease;
      this.oy = from.y + (oy - from.y) * ease;
      this.schedule();
  ...
  fit(animate = true): void {
    ...
    this.moveTo(
      scale,
      (w - m.bounds.width * scale) / 2 - m.bounds.x * scale,
      (h - m.bounds.height * scale) / 2 - m.bounds.y * scale,
      animate,
    );
  }
  native(): void {
  ...
  focusRegion(region: Rect): void {
    ...
    this.oy = this.canvas.clientHeight / 2 - (region.y + region.height / 2) * this.scale;
    this.schedule();
  }
  ...
  private draw(): void {
    ...
    if (this.focus) {
      ...
      ctx.setLineDash([5, 4]);
      ctx.strokeRect(
        this.ox + this.focus.x * this.scale,
        this.oy + this.focus.y * this.scale,
        this.focus.width * this.scale,
        this.focus.height * this.scale,
      );
      ctx.setLineDash([]);
    }
    this.changed(
