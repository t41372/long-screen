...
async function ffmpeg(args: string[], input?: Uint8Array): Promise<void> {
    ...
    const cmd = new Deno.Command('ffmpeg', { args: [...base, ...args], stdin: input ? 'piped' : 'null', stdout: 'inherit', stderr: 'inherit' });
...
const rawInput = ['-f', 'rawvideo', '-pixel_format', 'rgb24', '-video_size', `${width}x${height}`, '-framerate', String(fps), '-i', 'pipe:0', '-an'];
const out = (name: string) => new URL(name, root).pathname;
await ffmpeg([...rawInput, '-c:v', 'libx264', '-crf', '16', '-pix_fmt', 'yuv420p', '-bf', '2', '-g', '30', '-movflags', '+faststart', out('scroll.mp4')], raw);
await ffmpeg([...rawInput, '-c:v', 'libx264', '-crf', '18', '-pix_fmt', 'yuv420p', '-bf', '2', '-g', '15', '-movflags', 'frag_keyframe+empty_moov+default_base_moof', out('fragmented.mp4')], raw);
...
await ffmpeg([...rawInput, '-c:v', 'libx264', '-crf', '18', '-pix_fmt', 'yuv420p', '-bf', '2', '-g', '30', '-movflags', 'negative_cts_offsets', out('negative-cts.mov')], raw);
...
const truth = { width, height, fixedTop: 32, fps, frames: scenario.frames.length, path: scenario.layers[0].path.map(p => [p.x, p.y]), scenario: 'fixture', note: 'negative-cts-v0.mov is negative-cts.mov with the ctts version byte set to 0 (ReplayKit layout).' };
await Deno.writeTextFile(out('truth.json'), JSON.stringify(truth, null, 2));
await Deno.writeFile(out('world.png'), await encodeRGBA({ width: scenario.layers[0].world.width, height: scenario.layers[0].world.height, data: scenario.layers[0].world.data }));
