...
import { downscaleGray, analysisFactor } from '../src/core/raster.ts';
...
const probe = new Deno.Command('ffprobe', { args: ['-v', 'error', '-select_streams', 'v:0', '-show_entries', 'stream=width,height,nb_frames', '-of', 'csv=p=0', file], stdout: 'piped' });
...
const ffmpeg = new Deno.Command('ffmpeg', { args: ['-v', 'error', '-i', file, '-vf', `select=not(mod(n\\,${every}))`, '-fps_mode', 'vfr', '-f', 'rawvideo', '-pix_fmt', 'rgba', 'pipe:1'], stdout: 'piped', stderr: 'inherit' }).spawn();
const reader = ffmpeg.stdout.getReader(), frameBytes = width * height * 4;
let buffer = new Uint8Array(0), previous: Gray | undefined, previousImage: RGBA | undefined, previousFeatures: Feature[] | undefined, lastField: MotionField | undefined, learner: LayerLearner | undefined, index = 0;
...
const internals = learner as unknown as { colGain: Float64Array; horizontalGain: Float64Array; informativeFrames: number; rowChange: Float64Array; colChange: Float64Array; nativeColChange?: Float64Array; nativeFrames: number };
const top = (arr: Float64Array, n: number) => [...arr].map((v, i) => [i, v / internals.informativeFrames] as const).sort((a, b) => b[1] - a[1]).slice(0, n).map(([i, v]) => `${i}:${v.toFixed(2)}`);
console.log('informative frames', internals.informativeFrames, 'strongest column gains (analysis px)', top(internals.colGain, 5).join(' '), 'row gains', top(internals.horizontalGain, 5).join(' '));
