...
Open `http://localhost:4173`. The “Play a sample recording” chip in the intro feeds the printer a sample recording that wanders around a page (`static/demo/sample.mp4`), or drop your own screen recording onto its screen.
...
Drop a screen recording on the printer's screen → optionally set independent motion regions under Settings → Print → select a canvas and check the quality mask / notes → take the PNG. The recording can move along any path — scroll, pan, diagonal, loop back, pause, shake — as long as it shows every part you want in the picture.
...
src/synthetic   Synthetic scenes, renderer, ground-truth validator (used by the tests)
