...
- `keyframes.ts` — 关键帧原分辨率纹理片存取、回访检索；候选的 audit/精修核算法在 Rust（一次 `core().keyframesEvaluateCandidates` 调用），但排序/最强候选/对手歧义判定与置信度的 `Math.exp` 收尾刻意留在 TS（排序键就是 host-`Math.exp`'d 的置信度本身，位精确要求），详见十二节 track 行。TS 外壳：编排，混合绑定转发。
...
| 跟踪判定（里程计、重定位、漂移控制、各类 verdict）与关键帧候选评分                             | Rust（`rust/core/src/track.rs`，binding 拆成 `abi/track.rs` + `abi/track_{odometry,reacquire,keyframes}.rs`） | 曾测量并**未构建**有状态融合跟踪器：0.mov/e.mov 求解阶段耗时差异落在噪声范围内。以下几处**刻意**留在 TS，各自理由：<br>· `Math.exp`（`driftCorrection`/`reacquire`/`odometry` 的置信度收尾，及 `keyframes.ts` 关键帧评分的置信度公式，`src/core/keyframes.ts:116`）：V8 的 exp（llvm-libc）与 Rust libm 的 exp 在部分输入上相差 1 ULP，而与迁移前 TS 输出逐位一致是硬要求；切到 Rust exp 是一个独立、需要单独验证的行为变更，本轮不做。<br>· `keyframes.ts` 的候选评分/排序/最强候选选择/对手歧义判定（`src/core/keyframes.ts:124-146`）：排序键就是上面那个 host-`Math.exp`'d 的置信度本身，为保住位精确必须留在同一侧；`.8`/`6`（对手分数须在最强候选 80% 以内、或落点相差 ≥6px 才算歧义）是实测阈值，不是算法核。<br>· `keyframes.ts` 的 top-12 候选、每个至少 3 票（`src/core/keyframes.ts:227-231`）：视觉词投票计数的 posting-list 整理胶水，不是核算法。<br>· `region-step.ts` 的漂移门 `.12`（`scan.field.difference >= .12`，`src/pipeline/solve/region-step.ts:313`）：只是决定要不要调用 `driftCorrection` 这个 Rust 融合调用，不是跟踪数学本身，仍在调用点旁边。<br>· `solve/track.ts` 的 `isTextured`/`priorMatchesOf`/`gate()`：外壳自身的分发胶水（`isTextured` 是裸 `.length >= 8`；`priorMatchesOf` 是对已在 Rust 的 `matchFeatures` 的匹配胶水；`gate()` 是六路分支，没有自己的原生/patch/特征运算）。`ownFeaturesOf` 用于过滤特征的 `regionContains` TS 副本已删（R6-B）：过滤逻辑现在是 `rust/core/src/region.rs::filter_features`（泛型 `Region<M: AsRef<[u8]>>`，供拥有/借用两种掩码复用），`ownFeaturesOf` 现在只是对 `core().filterFeatures` 的薄调用。<br>· `src/media/convert.ts` 的 `agreement()`：浏览器端解码转换的健全性检查（比较直转与 worker 转换两条路径是否一致），不是重建算法，见下方"帧转换"行。<br>· `pose-graph.ts` 的 `correction()`：见上面 pose-graph 这一行。 |
