...
第三遍  按校正后的坐标做原分辨率瓦片合成，记录稀疏争议块
按需第四遍 顺序回放争议区原生候选，再做可见性与共同时间选择
```

后面的回访约束可以修正前面的轨迹，同时不必把所有完整帧留在内存。有争议区域时再顺序解码一次，在最终坐标上保存候选；无争议时不加这一遍。候选分析只读取落盘的稀疏数据，不对每个patch重新seek。代价是三至四次解码和多次磁盘访问，**不是实时处理承诺**。
...
角点的固定3×3结构张量用三列循环行缓冲与28行评分带计算，保留逐cell选点、同分顺序、ROI和BRIEF描述子。中间张量与评分缓冲从`24×(W+1)×(H+1)`字节降到`260×W`字节；这不是进程RSS。无可信配准时，静止判定遍历分析ROI内全部有效像素，平均绝对差阈值仍为5；空掩码判失追；两帧任一侧有纹理的有效像素另计均值，至少12个纹理像素且均差超过5也判失追，防止稀疏页面的空白稀释真实移动（e.mov的第823帧）。仅在累计差已超过`5×ROI最大像素数`时提前判失追，不能提前判静止，避免固定7像素网格漏看变化。
...
随后在**原像素**上精修并复核：`refineNative` 返回整数位移、残差和“离胜者两像素以外的最好残差”，后者小就说明存在周期性歧义。候选排序带一个弱的匀速先验，用来在周期别名之间打破平局。取样点由 `select_native_points` 决定：把采样窗口按 `step × step` 网格分格，每格取一个点，而不是固定取格子角上的那个网格点。`track::odometry` 把这一步交给分析灰度图引导，按由粗到细两级选点：先在格子里选**分析尺度梯度最强的那个分析像素**（该像素对应的原生像素块与格子相交、块中心又通过掩码时才参与比较），再只在这一个分析像素对应的原生像素块内取**原生梯度最强的那个原生像素**（同级并列都取行主序中最先出现的）；格子里没有任何候选、或选中原生像素的梯度不超过阈值，该格不贡献点。没有引导（`refineNative` 的 ABI 入口、`ls_refine_native` 不带 `guide` 时）则退化为对整个格子做满分辨率扫描、直接取原生梯度最强的像素——引导以降采样因子 1 调用时，每个分析像素对应恰好一个原生像素，两条路径逐位相同。取样点只取决于当前帧、区域、掩码与引导，与具体候选位移无关，因此 `track::odometry` 对同一区域最多 6 个候选只选一次、复用给每次精修。
...
位置图先尝试单锚点简单环的解析解：每个节点恰有两条对称、正有限权重的邻边，整图连通，沿环的闭合误差按边权倒数分配。所有二维残差都不超过Huber阈值6时才采用结果；其他拓扑和超阈值结果交回原有Gauss–Seidel求解，不改动回环测量本身。
...
兼容观察之间的清晰度评分取RGB三个通道、水平/垂直两个方向中央差分的最大值，保持每像素0…255、加分上限12与替换裕量4。冲突冻结与时间分量完整性要求不变。评分只在同一次新运行中比较：恢复旧打印只读回既有结果，不在旧瓦片上继续重建，因此不会混用新旧评分。
...
`rust/core/src/voting/screen.rs`补充独立的屏幕运动证据：同一屏幕位置的3×3灰度patch保持一致，而按页面运动对齐的完整patch不一致，且至少有两个彼此相隔Dmin的观察支持，才记入`screen`位图。重复停留不增加支持；屏幕patch容差从片源噪声导出，页面patch另保留分析网格相位余量。旧投票里的“邻域任意一点颜色接近”仍只作为原有弱证据，不作为这条新证据的正面证明。

宽悬浮面板由连续的宽行证据推断主体及邻近阴影范围。行内的大间隙会分开toast与悬浮按钮；占满视口宽度的工具列不走这个扩展。每个移动区域至多保留一个面板身份：最多96个获得三次独立支持的纹理锚点与一张屏幕坐标位图，只有至少90%锚点保持外观时才沿用范围。初次识别也回填仍在环中的同外观帧；不会重新解码或持有原生历史帧。模板不是任意遮挡的通用检测器，阴影范围也是有界启发式。自由移动的物体、超大透明遮挡、不同内容版本仍可能留下残片或不确定像素。

原生一致性掩码增加值2表示屏幕遮挡，0仍是普通不一致，1仍是接受或无证据。有屏幕证据的邻居不再否决刚露出的页面；相邻帧同屏位置的原生RGB保持片源噪声范围内时可以延续遮挡证据，防止比较次数不足的晚到遮挡冒充干净来源。新观察与已存像素一致时，值2可以突破整块相同和不替换短路，降级已存像素；压缩源允许片源噪声内的匹配，普通值0仍要求逐位相同。只改证据也必须dirty并落盘。时间分量完整性只接受值1，值2不能进入整块覆写路径；首次看到的内容仍写入并标记，固定框架仍由原来的固定区域与presentation负责。

### 位移展开一致性投票

为弥补 ±1 帧比较对"覆盖物尺寸超过单帧滚动速度"这类几何的失效，`solve()`（`src/pipeline/solve/solve.ts`）额外维护一份跨越更大时间跨度的证据：一个按分析分辨率保存的环形缓冲（`Ring`，`rust/core/src/voting.rs`），每帧保留各移动区域当时的画布 id / pose、一份区域自有的box本地灰度，以及投票计数和压缩的屏幕支持计数（`Ring::box_gray`，见下），按字节数定 budget（约 24MB，不保留原生分辨率帧）。对每个移动区域这一帧的最终 pose，从环里挑选最多 6 个满足"世界位移 ≥ Dmin"的历史帧作为参照——Dmin = max(64, 0.25×min(区域宽, 区域高)) 原生像素，它分散观察位置，但不保证大于任意覆盖物；特别宽大或长时间遮挡仍可能不可辨识。
...
第三遍的快速选择仍负责随时可恢复的打印前缀。原生合成循环同时标记16×16的分歧或遮挡块；索引向周围扩一块，按最终canonical canvas与pose组织为256×256的稀疏shard。`src/pipeline/sources.ts`在呈现与金字塔之前执行一次顺序回放。每块保留四个热候选，溢出候选全部写入档案，不因停留长、出现少或缓存淘汰而删除。相同原生帧和相同pose才合并观察span；停留不增加独立可见证据，消失后重现保留新时段。

`rust/core/src/sources/objects.rs`以页面、屏幕及局部平移竞争解释变化，先提出残差区域，再用原生纹理与外观轨迹确认。独立物体的实际像素范围与角色可回填到早期候选；暂停、反向、消失、跨pane与解除固定由观察时段记录。页面内动画的世界锚定轮廓或闭合容器支持PageDynamic，指针穿过它时遮挡证据优先。固定框架仍由原有固定画布与presentation保留一次；不把所有屏幕静止内容全局裁掉。每个tracker最多64个身份与8MiB表面缓存，超限明确报告，不谎称无遗漏。

可见性独立于输出颜色：Unknown、Visible、Occluded、Outside，以及受保护的placement遮挡。曾独立移动的物体暂时跟上页面速度，不构成页面干净来源的证据；保留其已确认表面的遮挡身份，只有覆盖整个身份历史的页面容器证据可以撤销早先假设。一次有运动和外观支持的可见来源可以胜过很多遮挡帧；未检出遮挡不等于Visible。静态块允许不同原生来源补齐各自干净的部分；含实际内容变化的连通分量统一选择一个观测epoch，不能把110/101/011拼成111。没有共同完整状态时保留partial与来源，而不是声称得到完整快照。选择共同epoch时，优先采用实际拍到整个分量的时刻，再比较完全可见块数与其他支持，避免为了多一个干净块而留下本可避免的空洞。完整观察覆盖仍不等于全部干净，Unknown和遮挡像素继续标为partial；不同epoch的内容不会因此混合。压缩、共同亚像素采样相位及色度取样差异只用于判定歧义，不插值生成输出。色度变化必须同时满足公共3×3亮度支持稳定和双向原生颜色邻域范围，不能把新颜色或消失的原生笔画当作采样误差。

出现替换争议时，另按档案核对选中外观。已有至少三个不同16px位置单元支持的干净来源，不会仅因后来遮挡中出现相似颜色而被否定；这个位置支持与带可见性状态的候选排序支持分开，停留中的遮罩变化不能增加位置票。对尚无这种独立支持的线索，只有后续确实被判为遮挡的原生观测，在至少64个可比像素的块内形成至少8个相连、颜色相符的矛盾像素，才把早先的页面运动正面线索降为歧义。匹配按像素限定，不扩张遮挡范围，也不生成颜色。若原打印的相同字节仍有未被否定的真实来源，则保留它；被确实否定的原来源仍须修复。没有这样的反证，干净来源照常胜出。这个检查在透明度学习前完成，已被反证的正面线索不能再作透明度模型的背景真值。已确认独立物体的边界范围还可撤销早期过渡帧中的页面运动正面线索：没有原生外观匹配的部分降为Unknown，不能仅凭边界框断言遮挡。检测要等到独立残差出现，因此只在同一pane的世界坐标内，向物体最初确认时段之前回看最多四帧，撤销边界范围内过强的正面判断。若同一身份先被判为PageDynamic、后来出现独立运动，早期归属降为Transition；它不产生遮挡像素，也不改变RGBA。对噪声容差内的两个Unknown颜色，至少三个不同footprint的逐字节重复可以打破原合成器的锐度平局；它只选择真实来源，不提升为Visible，也不能压过一次可靠干净观察。屏幕边缘的静止细条不能借同一16像素格内的页面文字获得正面运动证据。窄条必须有实际横向边界；触及图像裁切边缘时只接受很薄的残段，避免把页面的宽空白边距当成物体。同一原生颜色的连续窄条可共享运动不确定性，防止透过它的文字让中间像素被误标成页面；这只撤回Visible，不生成遮挡mask或修改颜色。

已确认物体的半透明边缘由`opacity.rs`在64×64场上学习`observed = transmission × background + bias`。背景必须来自独立Visible来源；保存实际帧号、世界位置、样本与反例，并检查方差、残差、物理颜色范围及不确定度。模型仅判遮挡，不生成页面颜色；有独立样本支持的负面模型也可以推翻先前的局部页面一致判断。低对比、不足以识别或没有干净观察的像素保持明确的不确定理由。

TS只负责缓存、顺序遍历与KV事务；来源选择、运动、透明度、动态连通性都在Rust。LZ4+Postcard归档与时间候选页支持快速频繁读取；角色时段建立共享原生索引，不为每个候选页重复传送全部角色；所有动态epoch先规划，再让每个shard扫描一次。候选、模型、模型系数与物体证据缓存分别按字节限额，最大单个页和编码瞬时缓冲另计，`source-summary`记录实际峰值、磁盘档案字节与阶段耗时。配置预算不等于浏览器总RSS上限。

自动分层中的小型内部固定区域保留独立的父层候选。Rust只为唯一包围它的移动区域建立替代标签；贯穿边框和手动划定的区域保持原归属。主追踪与辅助追踪分开，辅助掩码不修改正文证据；仅有页面运动支持的候选可以作为不确定的后备来源，且不能凭此新增未观察正文。页面动态角色只在本pane内成立，真正独立的覆盖物仍可跨pane。这样不会把固定区域永久丢弃，也不会把其中所有按钮、文字都无条件投影到正文。

狭长物体的原生掩码区分与外部相连的背景及内部前景。背景必须得到三个侧边（其中两个为真实外侧）的颜色支持；对象内封闭的同色图示不会被剔除。对占据主要面积的同色狭长前景，只沿实际原生颜色及已有前景连接补齐运动取整造成的缺口，不能跨过背景边界。大范围透明度仍由独立背景样本的模型判断。

自动固定孤岛的替代标签与最终位姿也进入争议索引，避免“干净帧被固定遮罩排除、留下的污染帧彼此一致”绕过来源回查。同色原生表面必须连接到物件范围之外的页面运动纹理；物件范围加4px的拟合不确定边缘不能自行供应这项证据。背景归属仍为Unknown等级，只打破其他Unknown候选的平局，不计入动态分量的Visible数。辅助追踪可用下一帧的真实同世界匹配补足前一帧新进入视口的表面证据，并保留被该表面包围、至多单侧裁切的小型原生细节；细节本身仍受物件负证据约束。phone尾端增加不依赖学习atlas的逐像素原始影格验收，避免把被错误归为固定区域的干净观察漏算为不可见。

每次应用以一个物理瓦片、该瓦片来源映射、画布计数作为同一事务提交。停止或写入失败保留已提交结果与`interrupted`状态；失败的修改不留在瓦片缓存里等待错误恢复时误写。某个shard的所有瓦片事务成功后，删除其可重算analysis/options/blocks临时行，保留原生档案、共同epoch和逐像素来源。`source-provenance`逐像素记录真实frame与reason，存在时优先于初次渲染的块owner和temporal记录。旧`provisional`仍表示世界一致性；新reason是另一种证据描述，不是概率，也不能用provisional下降代替像素真值验证。未发生像素改变的旧证据不被任意重写。
...
- **原生精修按格取点，不是网格点本身**（`rust/core/src/motion.rs::select_native_points`）。取网格点只在节点像素恰好落在边缘上时才采到样：稀疏的暗色页面上，边缘处处不在节点上，`< 12` 个样本触发 `Infinity`，真实步长被当成「静止」丢弃（`e.mov` 实测约 1600 个节点里只留 9–11 个）；与此相反，一条与网格行/列对齐的屏幕固定线（悬浮滚动条）则会在该行/列的每个节点上都被采到，其残差压过其余页面内容，导致真实位移被拒绝或被误差更低的周期别名取代（`d.mov`/`e.mov` 实测：水平滚动条正好落在某一网格行，46 个样本里 22 个来自这一行）。`track::odometry` 用分析灰度图由粗到细选点（格内先选分析尺度梯度最强的分析像素，再在它对应的原生像素块内选原生梯度最强的原生像素）而不是对整个格子做满分辨率扫描——降采样因子越大，每格要扫的原生像素就越少；因子为 1 时两条路径逐位相同，这一点由专门的回归测试和奇偶校验测试守护。
...
- **`identical === count`短路必须保留未修复provisional和独立screen证据两个例外**；普通成对不一致仍不能单独推翻整块复现（§七）。
- **普通不一致的降级判据必须逐位相同。**只有独立screen证据允许片源解码噪声内的匹配；它不能泛化到普通两帧分歧，否则正常内容会被反复降级（`tests/unit/compositor.test.ts`覆盖两侧）。
...
- 候选地址只使用最终canonical placement；档案中的原生RGBA不平均、不补画。
- 遮挡证据不得通过合并候选泄漏到另一个pose或外观时段；四个热候选是缓存上限，不是历史候选上限。
- 动态分量必须共享真实epoch；完整性的分母是曾观察到的像素集合，不是固定256。
- 存档分页顺序不能决定可见性支持。在反驳候选前重新核对完整候选集；已Visible的同色观察可得到其他页面表面的原生位姿佐证，暂停或同一pose的掩码变化不能增加独立支持。透明度分类改变证据后，必须重新核对并选择来源，但不能用自己的新结论再训练一轮模型。
- 被反驳的背景归属仍是Unknown，不能冒充该帧被直接观察为遮挡。选择时降低该假设的优先级，并禁止同样被反驳的旧基线强行回填。连贯遮挡证据的最小面积与需要更改的像素数是两回事；充分证据也可以只修一个区块边缘像素。
- 同色表面需要分别统计可用观察与已判遮挡观察的独立位姿，不能把两者混成次数投票。少量背景支持或至少同样多的独立遮挡支持，可以触发外观反证；少数同色遮挡不能否定在许多位置出现的正文背景。floating的30级代码背景与输入框底色相同，曾因此回填22级阴影。
- 页内动态内容的正面证据不能覆盖独立物体的负面证据；透明度模型只用于可见性。
- 单瓦片像素、来源、计数必须同事务。错误后的通用flush不能把失败事务的像素写回。
- 连通背景只提供Unknown等级的归属支持。单次背景假设不能擦除在至少三个不同16px位姿桶中反复观察到的未被否定原生内容；替代背景自身有同等独立支持时，或新旧来源均有归属支持而新来源的独立支持更多时，才允许打破平局。支持按片源noise比较RGB，不能要求逐位相同；实录c的Star字形会因1–3级压缩波动漏算，继而被单次背景替换。Visible仍优先。
...
延迟来源域：`rust/core/src/sources/`分别包含候选/history、tile与archive、object motion与matching/container/surface、annotation、opacity、analysis/phase、scene epoch、materialize/export；ABI在`abi/sources.rs`及其子目录，TS封送在`src/core/wasm/sources*.ts`。`src/pipeline/sources.ts`控制回放与应用，`source-opacity.ts`调度模型页，`source-epochs.ts`批量调度共同epoch，`src/storage/sources.ts`和`source-models.ts`管理有界缓存，`src/export/sources.ts`提供可独立核对的PNG/JSON。
...
- `features.rs` — 角点检测（3×3结构张量最小特征值，整数逐行循环缓冲）与二值描述子。Rust 算法核心。
...
- `pose_graph.rs` — 单锚点简单环解析解与Gauss–Seidel位置图松弛，纯数组/CSR输入，不认识字符串 id。Rust 算法核心。
...
- `voting/screen.rs` — 屏幕运动证据、宽悬浮面板范围与外观身份。Rust算法核心。
