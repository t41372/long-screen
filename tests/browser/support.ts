...
export async function rebuildIfStale(): Promise<void> {
    ...
    if (built > sources)
        return;
    const build = await new Deno.Command(Deno.execPath(), { args: ['run', '--allow-read', '--allow-write', '--allow-run', '--allow-env', `${root}scripts/build.ts`], cwd: root, stdout: 'inherit', stderr: 'inherit' }).output();
    if (!build.success)
...
export async function harness(options: { viewport?: { width: number; height: number } } = {}): Promise<Harness> {
    ...
    const server = Deno.serve({ port: 0, hostname: '127.0.0.1', onListen: () => { } }, createHandler({ root: `${root}dist`, mounts: { '/fixtures/': `${root}tests/fixtures`, '/test_case/': `${root}test_case` } }));
    const base = `http://127.0.0.1:${server.addr.port}`;
    const browser = await chromium.launch({ channel: Deno.env.get('LONGSCREEN_CHANNEL') || 'chrome', headless: true, executablePath: Deno.env.get('LONGSCREEN_CHROME') || undefined });
    ...
    return { browser, page, base, errors, external, close: async () => { await browser.close(); await server.shutdown(); } };
}
