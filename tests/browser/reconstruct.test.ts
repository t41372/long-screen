...
interface LayerSummary { maxError: number; missing: number; invented: number; mismatched: number; contaminated: number; skipped: number; covered: number; fragments: number; mainCanvas: string }
interface BrowserRun { status: string; error?: string; frames: number; seconds: number; diag: Record<string, number>; codes: string[]; body: LayerSummary; header: { mismatched: number; conflicts: number; covered: number }; memory: { peakResidentTiles: number; tileCacheLimit: number }; projectId: string }
...
async function reconstruct(page: import('playwright').Page, source: string, analysis: number, tolerance: number): Promise<BrowserRun> {
    return await page.evaluate(async ([source, analysis, tolerance]: [string, number, number]) => {
        ...
        const engine = new kit.Engine(db, frames, { ...kit.DEFAULT_SETTINGS, analysisSize: analysis }, { progress: () => {}, diagnostic: (d: any) => { diag[d.code] = (diag[d.code] || 0) + 1; }, preview: () => {}, project: () => {} });
        ...
        return { status: project.status, error: project.error, frames: project.renderedFrames, seconds, diag, codes: [...new Set(diagnostics.map((d: any) => d.code))], body: { ...body, mainCanvas: body.mainCanvas.id, fragments: body.fragments.length }, header: { mismatched: header.mismatched, conflicts: header.conflicts, covered: header.covered }, memory: await store.get('memory-stats'), projectId: project.id };
    }, [source, analysis, tolerance] as [string, number, number]) as BrowserRun;
}
Deno.test({ name: 'browser: lossless synthetic fixture reconstructs with exact placement and pixel identity in Chrome', sanitizeOps: false, sanitizeResources: false, fn: async () => {
    ...
    try {
        ...
        assertEquals([r.body.maxError, r.body.missing, r.body.invented, r.body.mismatched, r.body.fragments], [0, 0, 0, 0, 0], JSON.stringify(r.body));
...
async function decodeBaseline(page: import('playwright').Page, name: string, tolerance: number): Promise<{ outside: number; meanAbs: number; meanSigned: number }> {
...
for (const name of ['scroll.mp4', 'scroll.mov', 'fragmented.mp4', 'scroll.webm', 'negative-cts-v0.mov'])
    Deno.test({ name: `browser: encoded ${name} reconstructs from real decoded frames with exact placement and coverage`, sanitizeOps: false, sanitizeResources: false, fn: async () => {
        const h = await harness();
        try {
            await h.page.goto(h.base + '/harness.html');
            await h.page.waitForFunction('!!window.longScreenKit');
            ...
            assert(rate <= baseline.outside + .002, `${name}: ${(rate * 100).toFixed(2)}% of covered pixels differ, decode floor is ${(baseline.outside * 100).toFixed(2)}%`);
...
Deno.test({ name: 'browser: real recordings reconstruct end to end (skipped when absent; LONGSCREEN_REAL=all runs every file)', sanitizeOps: false, sanitizeResources: false, fn: async () => {
    ...
    try {
        ...
        for (const name of selected) {
            const r = await h.page.evaluate(async (name: string) => {
                ...
                const db = await kit.Database.open(), source = await kit.openMedia(file), diag: Record<string, number> = {}, phases: Record<string, number> = {};
                let phase = '', started = performance.now();
                const engine = new kit.Engine(db, source, { ...kit.DEFAULT_SETTINGS }, { progress: (p: any) => { if (p.phase !== phase) { if (phase) phases[phase] = Math.round(performance.now() - started); phase = p.phase; started = performance.now(); } }, diagnostic: (d: any) => { diag[d.code] = (diag[d.code] || 0) + 1; }, preview: () => {}, project: () => {} });
                ...
                return { status: project.status, error: project.error, frames: project.frames, rendered: project.renderedFrames, seconds: Math.round((performance.now() - t0) / 100) / 10, phases, diag, info: { width: source.info.width, height: source.info.height, codec: source.info.codec, notices: source.info.notices }, canvases: canvases.map(c => ({ id: c.id, kind: c.kind, bounds: c.bounds, tiles: c.tileCount, observed: c.observedPixels, conflicts: c.conflictPixels, attachedTo: c.attachedTo })), main: main.bounds, memory: await engine.store.get('memory-stats'), preview: canvas.toDataURL('image/png') };
            }, name);
            const { preview, ...report } = r;
            console.log(name, JSON.stringify({ status: report.status, frames: report.frames, seconds: report.seconds, main: report.main, phases: report.phases }));
