...
import { MemoryKV, iterate, Namespace, type KV } from '../../src/storage/db.ts';
...
export async function runScenario(scenario: Scenario, settings: Partial<Settings> = {}, control?: (engine: Engine) => void): Promise<RunResult> {
    ...
    const handlers: EngineEvents = { progress: () => events.progress++, diagnostic: d => events.diagnostics.push(d), preview: () => events.previews++, project: () => events.projects++ };
...
export async function collectRun(db: KV, project: Project, scenario: Scenario, tileSize: number): Promise<VerifiableRun & { project: Project; store: KV; diagnostics: Diagnostic[]; codes: Set<string>; memory: RunResult['memory'] }> {
    ...
    return { scenario, project, store, canvases, observations, diagnostics, codes: new Set(diagnostics.map(d => d.code)), regions, atlas, memory, tileSize };
}
