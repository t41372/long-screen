...
import { World, makeWorld, constantFrames, type Scenario, renderFrame } from '../../src/synthetic/world.ts';
...
function sparseScenario(): Scenario {
    ...
    const path = [{ x: 0, y: 0 }, { x: 17, y: 51 }, { x: 38, y: 113 }, { x: 81, y: 179 }, { x: 149, y: 231 }, { x: 127, y: 189 }, { x: 79, y: 129 }, { x: 31, y: 69 }, { x: 0, y: 0 }, { x: 0, y: 0 }, { x: 0, y: 0 }, { x: 0, y: 0 }, { x: 71, y: 75 }, { x: 111, y: 135 }, { x: 81, y: 211 }, { x: 19, y: 275 }];
    return { name: 'sparse-four-sided-random-walk', description: 'High-resolution narrow content with wide white gutters and four fixed sides; odd 2D movements and exact duplicates.', width, height, background: [30, 45, 54], layers: [{ id: 'body', viewport, world, path }], overlays: [
    ...
    ], frames: constantFrames(path.length, 30), expect: { fragments: { body: 0 }, diagnostics: { present: [], absent: [] }, maxError: 0, status: 'complete' } };
}
Deno.test('takeover: high-resolution sparse 2D traversal preserves full pane, gutters, and all four frame sides', async () => {
    ...
    await Deno.writeTextFile('test-results/takeover-geometry.json', JSON.stringify({ verification: result, regions: run.regions.map(({ mask, cells, ...r }) => r), performance: await run.store.get('performance') }, null, 2));
    assertEquals(result.maxError, 0); assertEquals(result.missing, 0); assertEquals(result.invented, 0); assertEquals(result.mismatched, 0);
...
Deno.test('takeover: sticky toolbar evidence survives a pause but not a changed or removed toolbar', () => {
    ...
    const make = (dy: number) => {
        ...
        for (let y = 0; y < 64; y++) for (let x = 0; x < 600; x++) {
    ...
    const paused = make(35); paused.data[(390 * 600 + 42) * 4] ^= 1;
