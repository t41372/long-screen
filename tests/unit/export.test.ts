...
import { createTarget, cleanupExport } from '../../src/export/target.ts';
...
import { DEFAULT_SETTINGS, type CanvasMeta, type Project } from '../../src/types.ts';
/** A fake FileSystemFileHandle whose writable collects bytes in memory. */
function fakeHandle() {
    const parts: Uint8Array[] = [];
    let closed = false, aborted: unknown;
    const handle = { createWritable: () => Promise.resolve({ write: (d: Uint8Array) => { parts.push(d.slice()); return Promise.resolve(); }, close: () => { closed = true; return Promise.resolve(); }, abort: (e: unknown) => { aborted = e; return Promise.resolve(); } }) } as unknown as FileSystemFileHandle;
    return { handle, bytes: () => { const out = new Uint8Array(parts.reduce((s, p) => s + p.length, 0)); let o = 0; for (const p of parts) { out.set(p, o); o += p.length; } return out; }, get closed() { return closed; }, get aborted() { return aborted; } };
}
function zipEntries(bytes: Uint8Array): string[] {
    ...
    const at = Number(view.getBigUint64(bytes.length - 34, true)), cd = Number(view.getBigUint64(at + 48, true)), count = Number(view.getBigUint64(at + 32, true));
    let p = cd;
...
async function project(width = 40, height = 30) {
    const db = new MemoryKV(), p: Project = { id: 'p1', created: '', updated: '', name: 'x', settings: { ...DEFAULT_SETTINGS, tileSize: 64, memoryMB: 64 }, status: 'complete', frames: 1, renderedFrames: 1, canvasCount: 1, tiles: 1, observedPixels: 1, diagnostics: {}, regions: [] };
    const store = new Namespace(db, `run/${p.id}/`), tiles = new TileStore(store, 64, 64), meta: CanvasMeta = { id: 'c', layer: 'l', name: 'c', kind: 'moving', bounds: { x: -3, y: -2, width, height }, tileCount: 0, observedPixels: 0, uncertainPixels: 0, conflictPixels: 0, maxLevel: 0, fragment: 0, firstTime: 0, lastTime: 1 };
    for (let y = -2; y < -2 + height; y++)
        for (let x = -3; x < -3 + width; x++) {
            if ((x + y) % 5 === 0)
                continue;
            const t = await tiles.get('c', Math.floor(x / 64), Math.floor(y / 64)), lx = x - Math.floor(x / 64) * 64, ly = y - Math.floor(y / 64) * 64, i = ly * 64 + lx;
...
Deno.test('export: project ZIP64 contains tiles, coverage, quality, ledgers and an offline viewer', async () => {
    ...
    for (const required of ['manifest.json', 'index.html', 'README.txt', 'tiles/c/0/0_0.png', 'tiles/c/0/-1_-1.png', 'coverage/c/0_0.bin', 'quality/c/0_0.json', 'observations.jsonl', 'diagnostics.jsonl', 'poses.jsonl', 'pose-edges.jsonl', 'analysis.jsonl', 'temporal.jsonl', 'graph-summary.json', 'memory-stats.json'])
...
Deno.test('export: canvases beyond the compatible sheet size are split into overlapping native sheets with coordinates', async () => {
    ...
    const names = zipEntries(target.bytes());
    assert(names.includes('manifest.json') && names.includes('sheet_0_0.png') && names.includes('sheet_1_0.json') && names.includes('sheet_1_0.png'), names.join(','));
