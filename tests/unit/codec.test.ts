...
): Promise<{ bytes: Uint8Array; expected: Uint8ClampedArray }> {
  ...
  const width = 37,
    ...
    image: RGBA = {
      ...
      data: Uint8ClampedArray.from(
        ...
        (_, i) => i % 4 === 3 ? (Math.floor(i / 4 / width) < 10 ? 0 : 255) : Math.floor(random() * 256),
      ),
    };
...
Deno.test('png: streaming encoder validates dimensions and row counts', async () => {
  await assertRejects(
    async () => {
      for await (const _ of encodePNG(0, 1, [])) void _;
    },
    Error,
  ...
  await assertRejects(
    async () => {
      for await (const _ of encodePNG(4, 2, [new Uint8Array(16)])) void _;
    },
    Error,
    'Expected 2 PNG rows',
  );
  await assertRejects(
    async () => {
      for await (const _ of encodePNG(4, 1, [new Uint8Array(3)])) void _;
    },
    Error,
