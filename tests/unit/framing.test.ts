...
import { frameLayout, frameCoordinate, buildFramedCanvas } from '../../src/core/framing.ts';
import { encodeRGBA } from '../../src/codec/png.ts';
import { MemoryKV } from '../../src/storage/db.ts';
import { TileStore, covered, markCovered } from '../../src/storage/tiles.ts';
...
const meta: CanvasMeta = { id: 'world', layer: region.id, name: 'world', kind: 'moving', bounds: { x: -3, y: 7, width: 14, height: 12 }, tileCount: 2, observedPixels: 1, uncertainPixels: 0, conflictPixels: 0, maxLevel: 0, fragment: 0, firstTime: 0, lastTime: 1 };
...
Deno.test('framing: all four sides are shown once at native scale; extensions never repeat UI glyphs', () => {
    ...
    for (let y = 0; y < layout.height; y++) for (let x = 0; x < layout.width; x++) {
        ...
        if (p === null) content++; else if (p === undefined) extensions++;
        else { const key = `${p.x},${p.y}`; copied.set(key, (copied.get(key) || 0) + 1); }
    }
...
Deno.test('framing: pane geometry is never shrunk to a tiny observed content box', () => {
    ...
    assertEquals(layout.content.width, 8); assertEquals(layout.content.height, 6);
...
Deno.test('framing: raw pixels and holes survive negative origins; decorated pixels have no evidence coverage', async () => {
    ...
    raw.pixels.set([19, 29, 39, 255], i * 4); markCovered(raw, i); raw.dirty = true;
    await tiles.flush();
    ...
    assert(framed?.presentation); assertEquals(framed.observedPixels, 0);
    ...
    assert(!covered(t, 6)); assertEquals(t.pixels[6 * 4 + 3], 255);
...
Deno.test('framing: missing references, empty/attached canvases and full-frame panes do not fabricate a frame', async () => {
    ...
    for (const m of [{ ...meta, tileCount: 0 }, { ...meta, attachedTo: 'elsewhere' }]) assertEquals(await buildFramedCanvas(db, tiles, m, region, [region], async () => {}), undefined);
...
Deno.test('framing: explicit ignore regions are not reintroduced as reference chrome', async () => {
    ...
    assert(framed); assertEquals((await tiles.get(framed.id, 0, 0)).pixels[3], 0);
});
